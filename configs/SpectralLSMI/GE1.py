cfg = dict(
    ...
    # Dataset settings
    data_type = "LSMI",  # LSMI dataset with multi-illuminant support
    dataset_root = "./data/SpectralLSMI/",
    rgb_camera = "Canon R5",  # Must match SSF filename in ImageEngineering_SSFs/h5/
    spectral_camera = "Spectricity S1",  # Must match SSF filename in Multispectral_SSFs/h5/
    gt_type = "xyz",  # "xyz" or "srgb"
    
    # LSMI-specific settings
    lsmi_output_type = "RGB",  # "RGB", "MS", or "RGB+MS"
    rgb_size = (256, 256),  # (H, W) for RGB output
    ms_size = None,  # Not needed for RGB-only
    misaligned = False,  # No misalignment for this config
